...
max_iterations = 100  # or whatever number you want
iteration = 0

# Prepare output files
timestamp = time.strftime("%Y%m%d_%H%M%S")
rx_binary_file = f'rx_data_{timestamp}.bin'
rx_csv_file = f'rx_data_{timestamp}.csv'

try:
    # Open files for writing
    with open(rx_binary_file, 'wb') as bin_file, open(rx_csv_file, 'w', newline='') as csv_file:
        csv_writer = csv.writer(csv_file)
        csv_writer.writerow(['Real', 'Imag'])  # CSV header
        ...
        while True:
            ...
            bin_file.write(rx_buffer)
            
            # Save processed data to CSV
            for complex_sample in rx_data:
                csv_writer.writerow([complex_sample.real, complex_sample.imag])
            
            print(f"Iteration {iteration + 1}: TX samples: {num_tx_samples}, RX samples: {num_rx_samples}")
    ...
    tx_ch.enable = False
    sdr.close()
    print(f"Saved binary data to: {rx_binary_file}")
    print(f"Saved CSV data to: {rx_csv_file}")